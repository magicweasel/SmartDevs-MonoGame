...
namespace SmartDevsGame.ParticleSystem
{
	...
	public abstract class ParticleSystem : DrawableGameComponent
	{
		...
		public void AddParticles(Vector2 where)
		{
			...
			int numParticles =
				RandomHelper.Random.Next(minNumParticles, maxNumParticles);
		...
		protected virtual void InitializeParticle(Particle p, Vector2 where)
		{
			...
			Vector2 direction = (_useDirection) ? _direction : PickRandomDirection();

			// pick some random values for our particle
			float velocity =
				RandomHelper.RandomBetween(minInitialSpeed, MaxInitialSpeed);
			float acceleration =
				RandomHelper.RandomBetween(minAcceleration, maxAcceleration);
			float lifetime =
				RandomHelper.RandomBetween(minLifetime, maxLifetime);
			float scale =
				RandomHelper.RandomBetween(minScale, maxScale);
			float rotationSpeed =
				RandomHelper.RandomBetween(minRotationSpeed, maxRotationSpeed);
		...
		public void Draw(SpriteBatch spriteBatch, GameTime gameTime)
		{
			...
			foreach (var p in particles)
			{
				...
				spriteBatch.Draw(_texture, p.Position, null, color,
		...
		public override void Draw(GameTime gameTime)
		{
			...
			foreach (var p in particles)
			{
				...
				_spriteBatch.Draw(_texture, p.Position, null, color,
					p.Rotation, origin, scale, SpriteEffects.None, 0.0f);
			}
